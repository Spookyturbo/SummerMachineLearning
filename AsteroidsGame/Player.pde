class Player {
  ...
  boolean wantToShoot = false; //Used to tell when the human player shot during training
  ...
  void think() {
    ...
    if (backpropagation && !humanTraining) {
      float[] desiredOutputs = {0, (vision[0] != 0 && vision[0] > 0.02) ? 0 : 1, 0, (vision[0] != 0 && vision[0] > 0.02) ? 1 : 0};
      brain.train(vision, desiredOutputs);
    }
  ...
  //---------------------------------------------------------------------------------------------------------------------------------------------------------      
  //Used for checking vision and button presses during human playing to train the network
  ArrayList<float[]> getState() {
    //Update the vision array
    look();
    float[] inputs = vision.clone();
    float[] outputs = new float[] {(boosting) ? 1 : 0, (spin == -0.08) ? 1 : 0, (spin == 0.08) ? 1 : 0, 0};
    if(wantToShoot) {
       outputs[3] = 1;
    }
